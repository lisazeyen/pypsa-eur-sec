...
def build_swiss(year):
    ...
    return (pd.read_csv(fn, index_col=list(range(2))) /
            3.6).loc["CH", str(year)]
...
def build_idees(year):
    ...
    for ct in population.index:
        ...
        totals.loc[ct, "electricity residential space"] = df.loc[[
            10, 11], year].sum()
        ...
        assert df.iloc[34,
                       0] == "Energy consumption by fuel - Eurostat structure (ktoe)"
        ...
        totals.loc[ct, "electricity services space"] = df.loc[[
            11, 12], year].sum()
        ...
        assert df.iloc[50,
                       0] == "Heavy duty vehicles (Diesel oil incl. biofuels)"
        totals.loc[ct, "total heavy duty road freight"] = df.loc[50, year]

        # car efficiency unit is [kgoe/100km]!
        ...
        totals.loc[ct, "electricity rail passenger"] = df.loc[[
            16, 19, 20], year].sum()
        ...
        totals.loc[ct, "total domestic aviation"] = totals.loc[ct, [
            "total domestic aviation freight", "total domestic aviation passenger"]].sum()
        totals.loc[ct,
                   "total international aviation"] = totals.loc[ct,
                                                                ["total international aviation freight",
                                                                 "total international aviation passenger"]].sum()
    ...
    totals["passenger car efficiency"] = 10 * \
        totals["passenger car efficiency"]
    totals["district heat share"] = (totals[["Derived heat residential", "Derived heat services"]].sum(
        axis=1) / totals[["Thermal uses residential", "Thermal uses services"]].sum(axis=1))
    return totals
...
def build_energy_totals():
    ...
    clean_df.loc[in_eurostat,
                 "total international navigation"] = eurostat.loc[idx[in_eurostat,
                                                                      :,
                                                                      "Bunkers",
                                                                      :],
                                                                  "Total all products"].groupby(level=0).sum()
    ...
    for sector, eurostat_sector in [("residential", "Residential"), (
            "services", "Services"), ("road", "Road"), ("rail", "Rail")]:
        for fuel, eurostat_fuel in [
                ("electricity", "Electricity"), ("total", "Total all products")]:
            clean_df.loc[missing_in_eurostat,
                         "{} {}".format(fuel,
                                        sector)] = eurostat.loc[idx[missing_in_eurostat,
                                                                    :,
                                                                    :,
                                                                    eurostat_sector],
                                                                eurostat_fuel].groupby(level=0).sum()
        ...
        fuel = "electricity"
        for use in uses:
            avg = (clean_df["{} {} {}".format(fuel, sector, use)
                            ] / clean_df["{} {}".format(fuel, sector)]).mean()
            print(
                "{}: average fraction of {} for {} is {}".format(
                    sector, fuel, use, avg))
            clean_df.loc[missing_in_eurostat,
                         "{} {} {}".format(fuel,
                                           sector,
                                           use)] = avg * clean_df.loc[missing_in_eurostat,
                                                                      "{} {}".format(fuel,
                                                                                     sector)]

        fuel = "total"
        for use in uses:
            avg = ((clean_df["{} {} {}".format("total", sector, use)] -
                    clean_df["{} {} {}".format("electricity", sector, use)]) /
                   (clean_df["{} {}".format("total", sector)] -
                    clean_df["{} {}".format("electricity", sector)])).mean()
            print(
                "{}: average fraction of non-electric for {} is {}".format(sector, use, avg))
            clean_df.loc[missing_in_eurostat,
                         "{} {} {}".format(fuel,
                                           sector,
                                           use)] = clean_df.loc[missing_in_eurostat,
                                                                "{} {} {}".format("electricity",
                                                                                  sector,
                                                                                  use)] + avg * (clean_df.loc[missing_in_eurostat,
                                                                                                              "{} {}".format("total",
                                                                                                                             sector)] - clean_df.loc[missing_in_eurostat,
                                                                                                                                                     "{} {}".format("electricity",
                                                                                                                                                                    sector)])
    ...
    for sector in ["residential", "services"]:
        ...
        total_heating = (clean_df.loc["NO",
                                      "{} {}".format("total",
                                                     sector)] - clean_df.loc["NO",
                                                                             "{} {}".format("electricity",
                                                                                            sector)]) / (1 - elec_fraction)

        for use in uses:
            fraction = ((without_norway["{} {} {}".format("total",
                                                          sector,
                                                          use)] - without_norway["{} {} {}".format("electricity",
                                                                                                   sector,
                                                                                                   use)]) / (without_norway["{} {}".format("total",
                                                                                                                                           sector)] - without_norway["{} {}".format("electricity",
                                                                                                                                                                                    sector)])).mean()
            clean_df.loc["NO", "{} {} {}".format(
                "total", sector, use)] = total_heating * fraction
            clean_df.loc["NO", "{} {} {}".format(
                "electricity", sector, use)] = total_heating * fraction * elec_fraction
    ...
    print("Aviation")
    clean_df.loc[missing_in_eurostat,
                 "total domestic aviation"] = eurostat.loc[idx[missing_in_eurostat,
                                                               :,
                                                               :,
                                                               "Domestic aviation"],
                                                           "Total all products"].groupby(level=0).sum()
    clean_df.loc[missing_in_eurostat,
                 "total international aviation"] = eurostat.loc[idx[missing_in_eurostat,
                                                                    :,
                                                                    :,
    ...
    clean_df.loc[missing_in_eurostat,
                 "total domestic navigation"] = eurostat.loc[idx[missing_in_eurostat,
                                                                 :,
                                                                 :,
                                                                 "Domestic Navigation"],
                                                             "Total all products"].groupby(level=0).sum()
    ...
    for fuel in ["total", "electricity"]:
        ...
        if fuel == "total":
            selection = [fuel + " two-wheel"] + selection + \
                [fuel + " heavy duty road freight"]
        road = clean_df[selection].sum()
        road_fraction = road / road.sum()
        for i in road_fraction.index:
            clean_df.loc[missing, i] = road_fraction[i] * \
                clean_df.loc[missing, fuel + " road"]
    ...
    for fuel in ["total", "electricity"]:
        ...
        for i in rail_fraction.index:
            clean_df.loc[missing, i] = rail_fraction[i] * \
                clean_df.loc[missing, fuel + " rail"].values
    ...
    for destination in ["domestic", "international"]:
        ...
        for i in aviation_fraction.index:
            clean_df.loc[missing,
                         i] = aviation_fraction[i] * clean_df.loc[missing,
                                                                  "total " + destination + " aviation"].values
    clean_df.loc[missing,
                 "total aviation passenger"] = clean_df.loc[missing,
                                                            ["total domestic aviation passenger",
                                                             "total international aviation passenger"]].sum(axis=1)
    clean_df.loc[missing,
                 "total aviation freight"] = clean_df.loc[missing,
                                                          ["total domestic aviation freight",
                                                           "total international aviation freight"]].sum(axis=1)
    ...
    clean_df.loc["BA", missing] = clean_df.loc["BA", "total residential"] / \
        clean_df.loc["RS", "total residential"] * clean_df.loc["RS", missing]
...
def build_eea_co2():
    ...
    df = df.set_index(['Country_code', 'Pollutant_name',
                       'Year', 'Sector_name']).sort_index()
    ...
    emissions = df.loc[idx[cts, pol, year, e.values], "emissions"].unstack("Sector_name").rename(
        columns=pd.Series(e.index, e.values)).rename(index={"All greenhouse gases - (CO2 equivalent)": "GHG"}, level=1)
    ...
    emissions['industrial non-elec'] = emissions['total energy'] - emissions[['electricity',
                                                                              'services non-elec',
...
def build_co2_totals(year=1990):
    ...
    co2 = eea_co2.reindex(
        ["EU28", "NO", "CH", "BA", "RS", "AL", "ME", "MK"] + eu28)

    for ct in ["BA", "RS", "AL", "ME", "MK"]:
        co2.loc[ct,
                "electricity"] = eurostat_co2[ct,
                                              "+",
                                              "Conventional Thermal Power Stations",
                                              "of which From Coal"].sum()
        co2.loc[ct, "residential non-elec"] = eurostat_co2[ct,
                                                           "+", "+", "Residential"].sum()
        co2.loc[ct, "services non-elec"] = eurostat_co2[ct,
                                                        "+", "+", "Services"].sum()
        co2.loc[ct, "road non-elec"] = eurostat_co2[ct, "+", "+", "Road"].sum()
        co2.loc[ct, "rail non-elec"] = eurostat_co2[ct, "+", "+", "Rail"].sum()
        co2.loc[ct, "domestic navigation"] = eurostat_co2[ct,
                                                          "+", "+", "Domestic Navigation"].sum()
        co2.loc[ct, 'international navigation'] = eurostat_co2[ct,
                                                               "-", "Bunkers"].sum()
        co2.loc[ct, "domestic aviation"] = eurostat_co2[ct,
                                                        "+", "+", "Domestic aviation"].sum()
        co2.loc[ct, "international aviation"] = eurostat_co2[ct,
                                                             "+", "+", "International aviation"].sum()
        # doesn't include industrial process emissions or fuel
        # processing/refining
        co2.loc[ct, 'industrial non-elec'] = eurostat_co2[ct,
                                                          "+", "Industry"].sum()
        # doesn't include non-energy emissions
        co2.loc[ct, 'agriculture'] = eurostat_co2[ct,
...
def build_transport_data():
    ...
    # CH from
    # http://ec.europa.eu/eurostat/statistics-explained/index.php/Passenger_cars_in_the_EU#Luxembourg_has_the_highest_number_of_passenger_cars_per_inhabitant
