...
country_iso_dic.update({'Norway': 'NO', 'Iceland': 'IS', 'Montenegro': 'ME',
                        'Serbia': 'RS', 'Albania': 'AL', 'United Kingdom': 'GB',
...
for ct in missing_area_ct:
    averaged_data = pd.DataFrame(area_per_pop.value.loc[map_for_missings[ct]].mean()*ct_total[ct], columns=["value"])
...
if annualise_cost:
    cost_retro = cost_retro[["cost_fix", "cost_var"]].apply(lambda x:
                 x*interest_rate/(1-(1+interest_rate)**-cost_retro.loc[x.index, "life_time"]))
...
energy_saved = energy_saved.apply(lambda x: x*x.weight, axis=1).groupby(level=[0,1]).sum()
cost_res = cost_res.apply(lambda x: x*x.weight, axis=1).groupby(level=[0,1]).sum()
    
# %%
res = pd.concat([energy_saved[l_strength], cost_res[l_strength]], axis=1, keys=["dE", "cost"])
