...
def calculate_cfs(n, label, cfs):
    ...
    for c in n.iterate_components(
        n.branch_components | n.controllable_one_port_components ^ {
            "Load",
            "StorageUnit"}):
        capacities_c = c.df[opt_name.get(
            c.name, "p") + "_nom_opt"].groupby(c.df.carrier).sum()
...
def calculate_nodal_costs(n,label,nodal_costs):
    ...
    for c in n.iterate_components(n.branch_components|n.controllable_one_port_components^{"Load"}):
        ...
        if c.name == "Store":
            items = c.df.index[(c.df.carrier == "co2 stored")
...
def calculate_costs(n, label, costs):
    ...
    if label[1] != "opt":
        costs.loc[("links-added",
                   "capital",
                   "transmission lines"),
                  label] = ((costs_db.at['HVDC overhead',
                                         'capital_cost'] * n.links.length + costs_db.at['HVDC inverter pair',
                                                                                        'capital_cost']) * n.links.p_nom_opt)[n.links.carrier == "DC"].sum()
        costs.loc[("lines-added",
                   "capital",
                   "transmission lines"),
                  label] = costs_db.at["HVAC overhead",
                                       "capital_cost"] * (n.lines.length * n.lines.s_nom_opt).sum()
    else:
        costs.loc[("links-added",
                   "capital",
                   "transmission lines"),
                  label] = (costs_db.at['HVDC inverter pair',
                                        'capital_cost'] * n.links.p_nom_opt)[n.links.carrier == "DC"].sum()
...
def calculate_curtailment(n, label, curtailment):
    ...
    avail = n.generators_t.p_max_pu.multiply(
        n.generators.p_nom_opt).sum().groupby(
...
def calculate_supply(n, label, supply):
    ...
    for i in bus_carriers:
        ...
        for c in n.iterate_components(n.one_port_components):
            ...
            s = c.pnl.p[items].max().multiply(c.df.loc[items, 'sign']).groupby(
        ...
        for c in n.iterate_components(n.branch_components):
            ...
            for end in [col[3:] for col in c.df.columns if col[:3] == "bus"]:
                ...
                items = c.df.index[c.df["bus" +
                                        end].map(bus_map, na_action=False)]

                if len(items) == 0:
                    continue

                # lots of sign compensation for direction and to do maximums
                s = (-1)**(1 - int(end)) * ((-1)**int(end) *
                                            c.pnl["p" + end][items]).max().groupby(c.df.loc[items, 'carrier']).sum()
                s.index = s.index + end
                s = pd.concat([s], keys=[c.list_name])
                s = pd.concat([s], keys=[i])
...
def calculate_supply_energy(n, label, supply_energy):
    ...
    for i in bus_carriers:
        ...
        for c in n.iterate_components(n.one_port_components):
            ...
            s = c.pnl.p[items].multiply(n.snapshot_weightings, axis=0).sum().multiply(
                c.df.loc[items, 'sign']).groupby(c.df.loc[items, 'carrier']).sum()
            s = pd.concat([s], keys=[c.list_name])
            s = pd.concat([s], keys=[i])

            supply_energy = supply_energy.reindex(
                s.index | supply_energy.index)
        ...
        for c in n.iterate_components(n.branch_components):
            ...
            for end in [col[3:] for col in c.df.columns if col[:3] == "bus"]:
                ...
                items = c.df.index[c.df["bus" +
                                        str(end)].map(bus_map, na_action=False)]

                if len(items) == 0:
                    continue

                s = (-1) * c.pnl["p" + end][items].multiply(n.snapshot_weightings,
                                                            axis=0).sum().groupby(c.df.loc[items, 'carrier']).sum()
                s.index = s.index + end
                s = pd.concat([s], keys=[c.list_name])
                s = pd.concat([s], keys=[i])
    ...
    metrics = metrics.reindex(
        pd.Index(
            [
                ...
                "line_volume_DC",
                "line_volume_shadow",
                "co2_shadow"]) | metrics.index)

    metrics.at["line_volume_DC", label] = (
        n.links.length * n.links.p_nom_opt)[n.links.carrier == "DC"].sum()
    metrics.at["line_volume_AC", label] = (
        n.lines.length * n.lines.s_nom_opt).sum()
    metrics.at["line_volume", label] = metrics.loc[[
        "line_volume_AC", "line_volume_DC"], label].sum()
...
def calculate_prices(n, label, prices):
    ...
    # WARNING: this is time-averaged, see weighted_prices for load-weighted
    # average
...
def calculate_market_values(n, label, market_values):
    ...
    for tech in techs:
        ...
        dispatch = n.generators_t.p[gens].groupby(
            n.generators.loc[gens, "bus"], axis=1).sum().reindex(columns=buses, fill_value=0.)
    ...
    for i in ["0", "1"]:
        ...
        for tech in techs:
            ...
            dispatch = n.links_t["p" +
                                 i][links].groupby(n.links.loc[links, "bus" +
                                                               i], axis=1).sum().reindex(columns=buses, fill_value=0.)
...
def calculate_price_statistics(n, label, price_statistics):
    ...
    price_statistics = price_statistics.reindex(
        price_statistics.index | pd.Index(["zero_hours", "mean", "standard_deviation"]))
    ...
    price_statistics.at["zero_hours", label] = df.sum(
    ).sum() / (df.shape[0] * df.shape[1])

    price_statistics.at["mean",
                        label] = n.buses_t.marginal_price[buses].unstack().mean()

    price_statistics.at["standard_deviation",
                        label] = n.buses_t.marginal_price[buses].unstack().std()
...
if __name__ == "__main__":
    ...
    if 'snakemake' not in globals():
        ...
        snakemake.config["run"] = "t_sink_fix_retro_together"
...
#
            ...
            # "flexible-onwind0p05-offwind0p05",
            # "flexible-onwind0p03-offwind0p03",
            # "flexible-onwind0p02-offwind0p02",
            # "flexible-onwind0p01-offwind0p01",
            # "flexible-onwind0p0-offwind0p0",
            # "flexible-offwind0p0",
            # "flexible-onwind0p0",
            # "flexible-solar0p0",
...
#            "2050_costs",
    ...
    networks_dict = {(cluster,lv,opt+sector_opt) :
                     snakemake.config['results_dir'] + snakemake.config['run'] +"/"+ snakemake.wildcards["year"] + '/postnetworks/elec_s{simpl}_{cluster}_lv{lv}_{opt}_{sector_opt}_{tsink}.nc'\
                     .format(simpl=simpl,
                             cluster=cluster,
                             opt=opt,
                             lv=lv,
                     ...
                     for tsink in snakemake.config['scenario']['Tsink']    }
    print(networks_dict)

    # os.chdir("/home/ws/bw0928/Dokumente/pypsa-eur-sec/")
    costs_db = load_costs(
        Nyears=1.,
        tech_costs="data/costs_old.csv",
